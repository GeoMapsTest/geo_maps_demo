...
async function discoverScans(item){
  const daoScans = (item.daos || []).map(d => ({
    href: d.href,
    title: d.title || "",
    attribution: d.label || d.localtype || ""
  })).filter(d => d.href);
  if(daoScans.length) return daoScans;

  const base = `scans/${encodeURIComponent(item.id)}`;
  const exts = [".jpg", ".jpeg", ".png", ".webp"];
  for(const ext of exts){
    const u = base + ext;
    if(await imageExists(u)) return [{ href: u, title: "", attribution: "" }];
  }
  const multi = [];
  for(let i=1;i<=10;i++){
    let found = false;
    for(const ext of exts){
      ...
      if(await imageExists(u)){ multi.push({ href: u, title: `Scan ${i}`, attribution: "" }); found = true; break; }
    }
    if(!found) break;
  }
  return multi;
  ...
  el.innerHTML = `
    <div class="card">
      ...
      <div class="kv">
        ...
        <div class="k">Physical note</div><div>${esc(item.physfacet || "(none)")}</div>
      </div>
      <div class="hr"></div>
...
async function renderViewer(item){
  ...
  viewer.innerHTML = "<div class='small'>Checking for digital surrogates…</div>";
  const scans = await discoverScans(item);
  if(!scans.length){
    viewer.innerHTML = "<div class='small'>No digital surrogates available yet for this item.</div>";
    return;
  }
  viewer.innerHTML = scans.map(scan => `
    <figure style="margin:0 0 16px 0">
      <img src="${esc(scan.href)}" alt="${esc(scan.title || `Scan for ${item.id}`)}">
      <figcaption class="small" style="margin-top:6px">
        ${scan.title ? `${esc(scan.title)}` : ""}
        ${scan.attribution ? `${scan.title ? "<br>" : ""}<span>${esc(scan.attribution)}</span>` : ""}
      </figcaption>
    </figure>
  `).join("");
}
